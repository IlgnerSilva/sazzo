...
import { verifyTwoFactorOTP } from "@/server/actions/auth/verifyTwoFactorOTP";

export function FormVerifyTwoFactorOTP() {
	const { translateMessage } = useMessageTranslation();
	const { isPending, executeAsync } = useAction(verifyTwoFactorOTP);
	const id = useId();
